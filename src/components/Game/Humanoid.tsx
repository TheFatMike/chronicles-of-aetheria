...
export const Humanoid = ({ color, isMoving = false, isGrounded = true, isAttacking = false }: HumanoidProps) => {
  ...
  useFrame((state) => {
    ...
    if (isAttacking) {
      // Attack animation (Quick swing with right arm)
      if (rightArm.current) {
        rightArm.current.rotation.x = -Math.PI * 0.8;
      }
  ...
  return (
    <group ref={body}>
      ...
      {/* Left Leg */}
      <group ref={leftLeg} position={[-0.15, 0.75, 0]}>
      ...
      {/* Right Leg */}
      <group ref={rightLeg} position={[0.15, 0.75, 0]}>
        <mesh position={[0, -0.3, 0]}>
          <boxGeometry args={[0.18, 0.6, 0.18]} />
          <meshStandardMaterial color="#333" />
        </mesh>
